...
int create_forward_message(char* dns_request, int request_len, struct sockaddr_storage source, char* serv)    // from https://www.binarytides.com/raw-udp-sockets-c-linux/
{
    ...
    */
    ...
    strncpy(data , dns_request, request_len);
    ...
    //UDP header
    udph->source = htons (((struct sockaddr_in*)source_addr)->sin_port);
    printf("source port is %d\n", udph->source);
    udph->dest = htons (53);
    udph->len = htons(8 + request_len); //tcp header size
